  	...
  	{if $viewtype neq "short"}
	    <span class="news-body-text">
	  		<span id="ls_contents-{$link_shakebox_index}">
	  				...
	  				{if $pagename neq "story"}  <a href="{$story_url}" class="morelink">read more</a> {/if}<br /> 
    	...
    	<span id="ls_comments_url-{$link_shakebox_index}">
			...
			{if $url_short neq "http://" && $url_short neq "://"}
				<span id="ls_trackback-{$link_shakebox_index}">
					<a href="{$trackback_url}" title="{$trackback_url}"><img src="{$my_pligg_base}/templates/{$the_template}/images/track.gif" align="absmiddle" /> {#PLIGG_MiscWords_Trackback#}</a>
				</span> | 
			{/if}
						
  		{if $tags ne ''}
  			<span id="ls_tags-{$link_shakebox_index}">
  				<b><a href="{$URL_tagcloud}" title="Tags" style='text-decoration: none;'><img src="{$my_pligg_base}/templates/{$the_template}/images/tag.gif" align="absmiddle" /> {#PLIGG_Visual_Tags_Link_Summary#}</a></b>: 
  		...
  		{if $Enable_AddTo eq 1}
  			| <a href="javascript://" onclick="
  								...
  								document.getElementById('addto-{$link_shakebox_index}').style.display = replydisplay;"><img src="{$my_pligg_base}/templates/{$the_template}/images/add.gif" align="absmiddle" /> {#PLIGG_Visual_LS_AddThisLinkTo#}</a>
  		{/if}

			{if $Enable_Recommend eq 1}
				{if $Recommend_Type eq 1}
					<span id="ls_recommend-{$link_shakebox_index}">
						| <a href="javascript://" onclick="show_recommend({$link_shakebox_index}, {$link_id}, '{$instpath}');"><img src="{$my_pligg_base}/templates/{$the_template}/images/email.gif" align="absmiddle" /> {#PLIGG_Visual_Recommend_Link_Text#}</a>
					</span> 
				{/if}
