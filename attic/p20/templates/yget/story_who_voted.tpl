...
<div id="comments">
    ...
    <div class = "whovotedwrapper" id = "idwhovotedwrapper">
        ...
        {section name=nr loop=$voter}
            ><li>{if $UseAvatars neq "0"}<span><img src="{$voter[nr].Avatar_ImgSrc}" alt="Avatar" align="absmiddle"/></span>{/if} <a href = "{$URL_user, $voter[nr].user_login}">{$voter[nr].user_login}</a><br/></li
        {/section}
        ></ol><br/>
    </div>
</div>
