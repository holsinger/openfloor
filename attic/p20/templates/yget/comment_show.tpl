...
<li>
  <div id="comment-wrap">
	...
	{* display user's avatar *}
	  ...
	  {* if you're viewing someone else's comment and if you haven't voted for it yet *}
      {if $comment_user_vote_count eq 0 && $current_userid neq $comment_author}
      | <span id="ratetext-{$comment_id}">{#PLIGG_Visual_Comment_Rate_Comment#}:</span> <span id="ratebuttons-{$comment_id}"> 
	...
	{* display comment content if the comment votes are greater than or equal to 0 *}
	  ...
	  <span id = "comment_content-{$comment_id}" style="display:none"> <br />
