...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
	...
	function _compile_if($arguments, $elseif = false, $while = false)
	{
		...
		preg_match_all('/(?>(' . $this->_var_regexp . '|\/?' . $this->_svar_regexp . '|\/?' . $this->_func_regexp . ')(?:' . $this->_mod_regexp . '*)?|\-?0[xX][0-9a-fA-F]+|\-?\d+(?:\.\d+)?|\.\d+|!==|===|==|!=|<>|<<|>>|<=|>=|\&\&|\|\||\(|\)|,|\!|\^|=|\&|\~|<|>|\%|\+|\-|\/|\*|\@|\b\w+\b|\S+)/x', $arguments, $_match);
		$_args = $_match[0];
		...
		for ($i = 0, $for_max = $count_args; $i < $for_max; $i++)
		{
			...
			switch (strtolower($_arg))
			{
				...
				case '(':
					...
					break;
				default:
					preg_match('/(?:(' . $this->_var_regexp . '|' . $this->_svar_regexp . '|' . $this->_func_regexp . ')(' . $this->_mod_regexp . '*)(?:\s*[,\.]\s*)?)(?:\s+(.*))?/xs', $_arg, $_match);
					if ($_match[0]{0} == '$' || ($_match[0]{0} == '#' && $_match[0]{strlen($_match[0]) - 1} == '#') || $_match[0]{0} == "'" || $_match[0]{0} == '"' || $_match[0]{0} == '%')
					{
						// process a variable
