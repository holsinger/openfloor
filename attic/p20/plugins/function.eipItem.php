<?php
	...
	function tpl_function_eipItem($params, &$smarty)
	{
		...
		// set some defaults
		if($eip_item['keyvaltype'] == ""){$eip_item['keyvaltype'] = "number";}
		if($eip_item['eip_type'] == ""){$eip_item['eip_type'] = "text";}
		if($eip_item['field_type'] == ""){$eip_item['field_type'] = "text";}
		...
		$js_data = "EditInPlace.makeEditable( {type: '".$eip_item['eip_type']."', pagename: '".$smarty->get_template_vars('eip_page_name')."', keyval: '".$eip_item['keyval']."', action: 'save', id: '".$params['eip_item']."',	save_url: '".$_SERVER['SCRIPT_NAME']."'";
