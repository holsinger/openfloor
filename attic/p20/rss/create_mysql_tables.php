<?php
	...
	$sql = 'DROP TABLE IF EXISTS `' . table_prefix . 'feed_import`;';
	$db->query($sql);	
	
	$sql = "CREATE TABLE `" . table_prefix . "feed_import` (
	  `feed_import_id` int(11) NOT NULL auto_increment,
	  `feed_id` int(11) NOT NULL,
	  `feed_freq_hours` int(11) NOT NULL,
	  `feed_votes` int(11) NOT NULL,
	  `feed_submitter` int(11) NOT NULL,
	  `feed_item_limit` int(11) NOT NULL,
	  `feed_category` int(11) NOT NULL,
	  `feed_url_dupe` tinyint(1) NOT NULL,
	  `feed_title_dupe` tinyint(1) NOT NULL,
	  `feed_status` varchar(255) NOT NULL,
	  PRIMARY KEY  (`feed_import_id`)
	) TYPE = MyISAM;";
	echo 'Creating table: \'feed_import\'....<br />';
	$db->query($sql);	
