...
class RSSImport {
	...
	function get_feed_import_list($feed_id=0){
		global $db;
		if(!isset($feed_id)){$feed_id = $this->FeedId;}
		$sql = "select * from " . table_prefix . "feed_import where feed_id = " . $feed_id;
		$feed_import_list = $db->get_results($sql);
		return $feed_import_list;
	}
