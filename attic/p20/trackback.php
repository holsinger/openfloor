...
if ( !empty($tb_url) && !empty($title) && !empty($tb_url) ) {
	...
	$dupe = $trackres->read();
	if ( $dupe )
		trackback_response(1, PLIGG_Visual_Trackback_AlreadyPing);
  
	$contents=@file_get_contents($tb_url);
	if(!$contents) 
		trackback_response(1, PLIGG_Visual_Trackback_BadURL);
	...
	if(!preg_match($pattern,$contents))
		trackback_response(1, PLIGG_Visual_Trackback_NoReturnLink);
