<? if($respondent['status'] == 'responding'): ?>
	<? if($past_vote < 1): ?>
		...
		<a href="javascript: <?= $this->userauth->isUser() ? 'var none = cpUpdater.ajaxParticipantVote(\'up\');' : 'window.location=\''.$this->config->item('custom_url').'/login/\';' ?>">
			<img src="images/ucp/not_answered_button.png" border="0" />
		</a>
	<? else: ?>
			<small>Let the respondent know that you don't feel that he/she is answering the question by clicking on the button below.</small><br /><br />
			<a href="javascript: <?= $this->userauth->isUser() ? 'var none = cpUpdater.ajaxParticipantVote(\'down\');' : 'window.location=\''.$this->config->item('custom_url').'/login/\';' ?>">
