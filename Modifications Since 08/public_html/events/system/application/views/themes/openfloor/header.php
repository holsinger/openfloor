<body onLoad='<?=$onload;?>'>
    ...
    <div id="wrapper">
		<div id="header">
			...
			<div class="box main-banner-wrapper  portalImg" id="zmid_22" style="float:left">
  				<div id="banner" class="portal-scope user-banner"> <img src="<?=$this->config->item('custom_theme_dir');?><?=$this->config->item('custom_theme');?>/logo_head.png" height="118">
  				
  				
  				</div>
			</div>
			<?php if ($this->userauth->isUser()):?>
					<div class="highlight_button1" style="float:right;margin-top:70px;margin-right: 20px;">
						<a>KARMA: <?=$this->session->userdata['user_karma']?></a>
					</div>
				<?php endif;?>
			<div class="box main-navigation" id="zmid_23" style="clear:both">
  				<ul id="mainnav" class="portal-scope">
    				<li class="selected ">
      					<a class="selected " href="<?= $this->config->site_url();?>">Home</a>
      					<? if ($this->userauth->isUser()) {?>
    					> <a class="selected " href="<?= $this->config->site_url() . 'user/profile/' . $this->session->userdata['user_name'];?>"><?=$this->session->userdata['user_name'];?></a>
    					<? } ?>
    				</li>
    				
					<li style="padding-left: 710px;">
						<? if ($this->userauth->isUser()):?>
							<a class="selected" href="<?= $this->config->site_url() . 'user/profile/' . $this->session->userdata['user_name'];?>">My Inbox <?=$this->session->userdata['inbox_count']; ?></a>
						<? endif; ?>
					</li>
  				</ul>
