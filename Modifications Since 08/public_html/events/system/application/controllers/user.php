...
class User extends Controller {
	...
	function login () {
		...
		if ($this->validation->run() == FALSE) {
			...
			$this->calculate_karma();
			$data['user_id'] = $this->user->user_id;
			$this->get_alert($data);
			$data['inbox_count'] = count($data['alert']['event']) + count($data['alert']['flagged']) + count($data['alert']['attention']);
			$data['user_karma'] = $this->user->get('user_karma');
			...
			$data['error'] = 'We couldn\'t log you in, the username or password may be incorrect, or you may still need to activate your account, or your account was freezed';
			$this->load->view('view_login',$data);
		}
	}
	...
	function calculate_karma()
	{
		...
		$user_array = @$this->user->get_stats($history_time, $this->user->user_id);
		//var_dump($user_array);
...
//		die();
		...
		$data['output'] .= "{$user_karma}\n<br />";
		$this->user->set_karma($this->user->user_id,$user_karma);
	...
	public function get_alert(&$data)
	{
		$alert = array();
		if($this->userauth->isAdmin()){
			//$this->event->getEventAlert();
			$event = $this->event->getEventAlert();
			$attention = $this->alert->getQuestionAttention($data['user_id']);
			//$private = $this->alert->getPrivateQuestions();
			$alert['event'] = $event;
			$alert['attention'] = $attention;
			//$alert['private'] = $private;
			$data['alert'] = $alert;
			log_message('debug', "getEventAlert:".trim($this->db->last_query()));
		}
		else if(!$this->userauth->isAdmin()){
			//$this->event->getEventAlert();
			$event = $this->event->getEventAlert();
			$flagged = $this->alert->getQuestionFlagged($data['user_id']);
			//$private = $this->alert->getPrivateQuestions();
			$alert['event'] = $event;
	...
	public function profile_ajax($user_id, $tab='1'){
		...
		if($tab == 1){
			...
			$data['explanation'] = "These are the questions you have asked.";
			...
			$data['explanation'] = "This is where you can see all the questions you've voted on.";
			$this->load->view('user/ajax_profile_vote',$data);
		}elseif ($tab == 4){
			//users last 10 comments
			$data['comments'] = $this->comment->getCommentsByUser($data['user_id']);
			foreach($data['comments'] as $k => $v) {
				$data['comments'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
				$data['comments'][$k]['question_name'] = anchor('question/view/' . url_title($v['event_name']) . '/' . url_title($v['question_name']), substr($v['question_name'],0,50) . '...');
			}
			$this->load->view('user/ajax_profile_comment',$data);
		}elseif ($tab == 5){
			//users last 10 flag question
			$data['flags'] = $this->flag->getFlagsByUser($data['user_id']);
			foreach($data['flags'] as $k => $v) {
				$data['flags'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
			}
			$data['explanation'] = "These are all the questions you have flagged.";
			$this->load->view('user/ajax_profile_flag',$data);
		}elseif ($tab == 6){
			//users last 10 needing attention
			if ($this->userauth->isAdmin())
			{
				$data['attention'] = $this->flag->getQuestionAttention();
				foreach($data['attention'] as $k => $v) {
					$data['attention'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
					$data['attention'][$k]['question_name'] = anchor('question/view/' . url_title($v['event_name']) . '/' . url_title($v['question_name']), substr($v['question_name'],0,50) . '...');
				}
				$this->load->view('user/ajax_profile_attention',$data);
			}else{
				$data['flags'] = $this->question->getQuestionsByUserFlag($data['user_id']);
				foreach($data['flags'] as $k => $v) {
					$data['flags'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
				}
				$data['explanation'] = "These are your questions that others have flagged.";
				$this->load->view('user/ajax_profile_flag',$data);
			}
		}elseif ($tab == 7){
			//users last 10 answered question
			$data['answered'] = $this->question->getAnsweredByUser($data['user_id']);
			foreach($data['answered'] as $k => $v) {
				$data['answered'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
				$data['answered'][$k]['question_name'] = anchor('question/view/' . url_title($v['event_name']) . '/' . url_title($v['question_name']), substr($v['question_name'],0,50) . '...');
			}
			$this->load->view('user/ajax_profile_answered',$data);
		}elseif ($tab == 8){
			//users last 10 private question
			$data['private'] = $this->question->getPrivateQuestions();
			foreach($data['private'] as $k => $v) {
				$data['private'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
				$data['private'][$k]['question_name'] = anchor('question/view/' . url_title($v['event_name']) . '/' . url_title($v['question_name']), substr($v['question_name'],0,50) . '...');
			}
			$data['explanation'] = "These are questions you asked that were marked private by you or someone else.";
			$this->load->view('user/ajax_profile_question',$data);
		}elseif ($tab == 9){
			if ($this->userauth->isAdmin()){
				$data['request_events'] = $this->event->get_student_request_event();
				foreach($data['request_events'] as $k => $v) {
					$data['request_events'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
				}
				$this->load->view('user/ajax_profile_event_request',$data);
			}
			if (!$this->userauth->isAdmin()){
				$data['request_events'] = $this->event->get_student_request_event($this->userauth->user_id);
				foreach($data['request_events'] as $k => $v) {
					$data['request_events'][$k]['event_name'] = anchor('forums/cp/' . url_title($v['event_name']), substr($v['event_name'], 0, 20) . '...');
				}
				$this->load->view('user/ajax_profile_event_request',$data);
			...
			if ($this->userauth->isAdmin())
			{
				$data['account'] = $this->user->getFreezeUser();
				$this->load->view('user/ajax_profile_user',$data);
			}
			if (!$this->userauth->isAdmin()){
				$data['feedback'] = $this->question->get_feedback($this->userauth->user_id);
				foreach($data['feedback'] as $k => $v) {
					$data['feedback'][$k]['question_name'] = anchor('question/view/' . url_title($v['event_name']) . '/' . url_title($v['question_name']), substr($v['question_name'],0,50) . '...');
				}
				$this->load->view('user/ajax_profile_feedback',$data);
			}
		}
	}
...
//		echo "</pre>";
		$glcid = $_GET['glcid'];
		$webctid = $_GET['webctid'];
		$name = $_GET['name'];
		$email = $_GET['email'];
		$status = $_GET['status'];
		$time = $_GET['time'];
//		echo "now:" . time();
		$mac = $this->calculateMac(array($glcid, $webctid, $email, $name, $status, $time),
		'openfloor');
//		echo "<br>";
//		print_r($mac);
//		echo "<br>";
//		print_r($_GET['mac']);
		if($mac != $_GET['mac'] || (intval($time) - time()) > 60)
		{
			die("Your MAC is Invalid or timeout!");
		}
//		ob_clean();
//		redirect('');
		$this->session->sess_destroy();
		if ($this->user->is_username($webctid))
		{
			$data['breadcrumb'] = array('Home'=>$this->config->site_url(),'Login'=>'');		
			$userid = $this->user->getUserIdByEmail($email);
			$this->user->login_user($webctid,$userid);
			$this->user->user_id = $userid;	
			$this->user->user_name = $webctid;
			$this->calculate_karma();
			$data['user_id'] = $userid;
			$this->get_alert($data);
			$data['inbox_count'] = count($data['alert']['event']) + count($data['alert']['flagged']) + count($data['alert']['attention']);
