...
cpUpdater.view_tab_section = function(tab_name, question_id, option_1, option_2){
	...
	if(tab_name == cpUpdater.current_tab_name && question_id == cpUpdater.current_question_id) {	
		...
		}else if(tab_name == 'flag'){
			url = site_url + 'flag/flagQuestion/' + question_id;
		}
...
cpUpdater.change_sort = function(_sort) {
	...
	if(sort == 'pending'){
		...
		$('question_title').innerHTML = "Promoted Questions";
	}else if(sort == 'private'){
		$('question_title').innerHTML = "Private Questions";
	}else if(sort == 'inappropriate'){
		$('question_title').innerHTML = "Inappropriate Questions";
	}else if(sort == 'duplicate'){
		$('question_title').innerHTML = "Duplicate Questions";
	}else{
		$('question_title').innerHTML = "Answered Questions";
	}
	// Change the highlights
	if(event_timing == 'past'){
		sort_links = ['pending', 'asked', 'deleted', 'duplicate', 'inappropriate', 'private', 'promot'];
	}else{
		sort_links = ['pending', 'newest', 'asked', 'deleted', 'duplicate', 'inappropriate', 'private', 'promot'];
	}
	...
	if(sort == 'pending' || sort == 'newest' || sort == 'asked' || sort == 'deleted') {
		ajax_update_url = site_url + 'forums/cp/' + event_name + '/upcoming_questions/' + sort;
		ajax_count_url = site_url + 'forums/cp/' + event_name + '/upcoming_questions_count/' + sort;
	}
	else{
		ajax_update_url = site_url + 'forums/cp/' + event_name + '/flagged_questions/' + sort;
		ajax_count_url = site_url + 'forums/cp/' + event_name + '/flagged_questions_count/' + sort;
	}
...
//==================================================================================
//= FlagQuestionOnSucess - Call Back function when flaging a question	     	   =
//==================================================================================
cpUpdater.FlagQuestionOnSucess = function(transport) {
	if(transport.responseText){
		//alert(transport.responseText);
		$('question_error_div').innerHTML = "Flagged Successfully!";
		eval('var response = '+transport.responseText);
		setTimeout('cpUpdater.view_tab_section("flag", '+response.question_id+')', 2000);		
		
	}else{
		$('question_error_div').innerHTML = "Could not flag!";
	}
}