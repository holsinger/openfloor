...
class Forums extends Controller 
{
	...
	public function cp($event, $ajax = null, $option_1 = null, $option_2 = null)
	{
		...
		log_message('debug', "_upcoming_questions_count:".trim($this->db->last_query()));
				break;
			case 'flagged_questions_count':
				if(isset($option_1)) $data['sort'] = $option_1;
				echo $this->_flagged_questions_count($data);
		log_message('debug', "$option_1._flagged_questions_count:".trim($this->db->last_query()));
				break;
			default:
			...
			$this->_getQuestionAnswerRate($data);
	...
	public function ajax_change_event_status($event_id, $status){
		switch ($status) {
			...
			case "finish":
		    	$timestamp = time();
				echo $this->event->update_event_form($event_id, array('event_finished' => '1', 'finish_time' => date("Y-m-d H:i:s",$timestamp)));
			    break;
			case "no_finish":
	...
	public function EditQuestion($question_id, $event_id, $option = ''){
		...
		if($option == 'update'){
			...
			if (!$this->question->haveAnswered($question_id, $this->userauth->user_id)){
				$changed = $this->question->insertAnswer(array(	'fk_question_id' => $question_id, 
																'fk_user_id' => $this->userauth->user_id,
																'answer' => $_POST['question_answer']));
			}else {
				$changed = $this->question->updateAnswer(array(	'fk_question_id' => $question_id, 
																'fk_user_id' => $this->userauth->user_id,
																'answer' => $_POST['question_answer']));
			}
			if($changed > 0){
		...
		// Do this on intial page load
			...
			$data['answer'] = $this->question->get_answer($question_id);
			$data['event_id'] = $event_id;
//			$data['alert'] = $alert;
	...
	**/
	...
	public function ShowAnswer($question_id){
		...
		$q_answers = $this->question->get_answers($question_id);
		$out = '';
		foreach ($q_answers as $k => $answer){
			$out .= '
			<div id="video_container" style="margin: 10px">
				' . $answer['user_name'] . ': ' . $answer['answer'] . '
			</div>
			'; 
		}
		echo $out;
...
EOT;
	...
	public function _flagged_questions_count(&$data)
	{
		//$this->question->question_status = isset($data['sort']) && $data['sort'] == 'asked' ? 'asked' : 'pending';
		if(isset($data['sort'])) {
			if($data['sort'] == 'promot'){
				$this->question->flag_reason = 'other';
				$this->question->flag_reason_other = 'promoted';
			} elseif ($data['sort'] == 'private') {
				$this->question->flag_reason = 'other';
				$this->question->flag_reason_other = 'private';
			} elseif ($data['sort'] == 'inappropriate') {
				$this->question->flag_reason = 'inappropriate';
				$this->question->question_status = 'deleted';
			} elseif ($data['sort'] == 'duplicate') {
				$this->question->flag_reason = 'other';
				$this->question->flag_reason_other = 'duplicate';
				$this->question->question_status = 'deleted';
			}
		}
		return $this->question->count_flagged_questions($data['event_id']);
	}
	...
	private function _flaggedQuestions(&$data)
	{
		$this->question->order_by = 'date';
		if(isset($data['sort'])) {
			if($data['sort'] == 'promot'){
	...
	private function _getQuestionAnswerRate(&$data)
	{
		if(count($data['current_question']) > 0)
		{
			$rates = $this->question->getAnswerRate($data['current_question'][0]['question_id']);
			foreach ($rates as $k => $rate)
			{
				$data['rates']['rate_' . $rate['fk_user_id']] = $rate['rate'];
			}
		}
	}
}
