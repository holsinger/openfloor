	...
	<table border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td valign="top" style="padding-left: 3px;">
				<h4>Classes: The user's classes listed out.</h4>
			</td>
		</tr>
		<?php foreach ($classes as $k => $class):?>
		<tr>
			<td valign="top" style="padding-left: 18px;">
				Class:<a href="<?=site_url('classroom/view/' . $class['classroom_id'])?>"><?= $class['name']?></a>&nbsp;&nbsp;(<?=$class['stu_count']?>)
			<td>
		</tr>
		<?php endforeach;?>
		<tr>
			<td valign="top">
			<input type="button" name="back_but" value="add a class wizard" id="back_but" onclick="window.location=site_url + 'classroom/add';" class="button">
			</td>
		</tr>
	</table>
</div>
...
<div id="content_div" class="event_content_div">
	...
	<? endif; ?>
	<? if (!$this->userauth->isAdmin() && $this->userauth->isUser()):?>
		<?="<p>".anchor('/event/create_event','Request an event').(isset($error)?"<br />".$error:'')."</p>"?>
	<? endif;?>
	<!-- FIRST, LIVE EVENTS -->
	<? $count=0; ?>
        ...
        <? if ($count < 10) : ?>
			<div id="working_town_div">
				...
				<div class="highlight_bottom">					
					<div class="highlight_button">
						<? if($this->userauth->isAdmin()): ?>
							<a href="<?= $this->config->site_url();?>event/create_event" alt="CREATE EVENT"><span>CREATE EVENT</span></a>
						<? endif; ?>
						<? if (!$this->userauth->isAdmin() && $this->userauth->isUser()):?>
						
							<a href="<?= $this->config->site_url();?>contact/showForm/request_an_event" alt="EVENT REQUEST"><span>EVENT REQUEST</span></a>
						<? endif;?>
					</div>
				</div>
			</div>
